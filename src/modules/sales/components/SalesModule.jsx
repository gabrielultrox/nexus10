...
function SalesModule({ saleId, viewMode, formResetToken, onOpenDetail, onOpenList }) {
  ...
  useEffect(() => {
    ...
    const unsubscribeCustomers = subscribeToCustomers(
      ...
      (error) => {
        setErrorMessage(
          (current) =>
            current || getFriendlyErrorMessage(error, 'Nao foi possivel carregar os clientes.'),
        )
        resolveCustomers()
      },
    )
    const unsubscribeProducts = subscribeToProducts(
      ...
      (error) => {
        setErrorMessage(
          (current) =>
            current || getFriendlyErrorMessage(error, 'Nao foi possivel carregar os produtos.'),
        )
        resolveProducts()
      },
    )
  ...
  const visibleSales = useMemo(() => {
    ...
    return sales.filter((sale) => {
      ...
      return (
        (statusFilter === 'all' || sale.domainStatus === statusFilter) &&
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      {
        label: 'Lancadas',
        value: String(posted.length).padStart(2, '0'),
        description: 'concluido',
        badgeText: 'ok',
        variant: 'success',
      },
      {
        label: 'Estornadas/canceladas',
        value: String(reversed.length + cancelled.length).padStart(2, '0'),
        description: 'em revisao',
        badgeText: 'revisao',
        variant: 'warning',
      },
      {
        label: 'Volume lancado',
        value: formatCurrencyBRL(volume),
  ...
  async function handleStatusChange(nextStatus) {
    ...
    try {
      ...
      setFeedbackMessage(
        `Venda ${selectedSale.code ?? selectedSale.id} atualizada para ${getSaleStatusMeta(nextStatus).label}.`,
      )
      playPdvSuccess()
    } catch (error) {
        ...
        <>
          ...
          <SurfaceCard title="Vendas postadas">
            <div className="sales-domain__toolbar">
              ...
              <div className="ui-field">
                <label className="ui-label" htmlFor="sales-status">
                  Status
                </label>
                <Select
                  id="sales-status"
          ...
          {showCreateLoading ? (
            ...
            <SalesFormPanel
              ...
              onTotalsChange={(field, value) =>
                setFormState((current) => ({
                  ...current,
                  totals: { ...current.totals, [field]: value },
                }))
              }
