...
function SalesDetailPanel({
  ...
  const canMutateSale =
    Boolean(acting || !canWrite || selectedSale?.domainStatus !== 'POSTED') === false
      ...
      <div className="sales-domain__detail-header">
        ...
        {selectedSale ? (
          <div className="sales-domain__detail-actions">
            ...
            {canMutateSale ? (
              <>
                <button type="button" className="ui-button ui-button--warning" onClick={onReverse}>
                  Estornar venda
                </button>
                <button type="button" className="ui-button ui-button--danger" onClick={onCancel}>
                  Cancelar venda
                </button>
              </>
            ) : (
      ...
      {!selectedSale ? (
        ...
        <div className="sales-domain__detail">
          <div className="commerce-detail-band">
            ...
            <span
              className={`ui-badge ${selectedSale.stockPosted ? 'ui-badge--success' : 'ui-badge--warning'}`}
            >
              {selectedSale.stockPosted ? 'Estoque publicado' : 'Estoque pendente'}
            </span>
            <span
              className={`ui-badge ${selectedSale.financialPosted ? 'ui-badge--success' : 'ui-badge--special'}`}
            >
          ...
          <div className="sales-domain__detail-grid">
            ...
            <div className="sales-domain__detail-card">
              ...
              <small>
                {selectedSale.source === 'ORDER'
                  ? `Pedido ${selectedSale.orderId ?? '-'}`
                  : 'Venda direta'}
              </small>
            </div>
            <div className="sales-domain__detail-card">
            ...
            <div className="sales-domain__detail-card">
              ...
              <small>
                {selectedSale.financialPosted ? 'Financeiro publicado' : 'Aguardando financeiro'}
              </small>
            </div>
            <div className="sales-domain__detail-card">
              <span>Criada em</span>
              <strong>
                {formatDateTime(selectedSale.createdAtDate ?? selectedSale.createdAt)}
              </strong>
              <small>
                Lancada em {formatDateTime(selectedSale.launchedAtDate ?? selectedSale.launchedAt)}
              </small>
            </div>
          </div>
          ...
          <div className="sales-domain__detail-panels">
            <div className="sales-domain__summary">
              <div className="sales-domain__summary-row">
                <span>Subtotal</span>
                <strong>{formatCurrencyBRL(selectedSale.totals?.subtotal ?? 0)}</strong>
              </div>
              <div className="sales-domain__summary-row">
                <span>Frete</span>
                <strong>{formatCurrencyBRL(selectedSale.totals?.freight ?? 0)}</strong>
              </div>
              <div className="sales-domain__summary-row">
                <span>Adicional</span>
                <strong>{formatCurrencyBRL(selectedSale.totals?.extraAmount ?? 0)}</strong>
              </div>
              <div className="sales-domain__summary-row">
                <span>Desconto</span>
                <strong>{formatCurrencyBRL(selectedSale.totals?.discountValue ?? 0)}</strong>
              </div>
              <div className="sales-domain__summary-row sales-domain__summary-row--total">
                <span>Total final</span>
                <strong>{formatCurrencyBRL(selectedSale.totals?.total ?? 0)}</strong>
              </div>
            </div>
          ...
          <div className="sales-domain__detail-panels">
            <div
              className={`sales-domain__timeline-card${selectedSale.domainStatus !== 'POSTED' ? ' sales-domain__timeline-card--muted' : ''}`}
            >
              ...
              <div className="sales-domain__timeline-list">
                <div className="sales-domain__timeline-step">
                  ...
                  <p>
                    {selectedSale.source === 'ORDER'
                      ? `Gerada do pedido ${selectedSale.orderId ?? '-'}`
                      : 'Venda criada diretamente'}
                  </p>
                </div>
                <div className="sales-domain__timeline-step">
                  <strong>Estoque</strong>
                  <p>
                    {selectedSale.stockPosted
                      ? 'Movimento aplicado no estoque'
                      : 'Ainda nao publicado'}
                  </p>
                </div>
                <div className="sales-domain__timeline-step">
                ...
                <div className="sales-domain__timeline-step">
                  ...
                  <p>
                    {selectedSale.launchedBy
                      ? `Lancada por ${selectedSale.launchedBy.name ?? selectedSale.launchedBy.email ?? 'operador'}`
                      : 'Sem operador informado'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="sales-domain__notes">
              ...
              <p>
                {selectedSale.orderId
                  ? `Pedido vinculado: ${selectedSale.orderId}`
                  : 'Venda criada diretamente.'}
              </p>
            </div>
          </div>
          ...
          <div className="sales-domain__items-list">
            <div className="sales-domain__items-header">
              ...
              <span className="ui-badge ui-badge--special">
                {selectedSale.items?.length ?? 0} itens
              </span>
            </div>
            {selectedSale.items?.map((item, index) => (
              <div
                key={`${item.productId || item.name}-${index}`}
                className="sales-domain__item-line"
              >
                <div>
