...
function ReportBarChart({ title, items, maxValue, valueFormatter, metaFormatter }) {
  const resolvedMaxValue =
    maxValue || Math.max(...items.map((item) => item.value || item.total || item.quantity || 0), 1)
        ...
        <div className="report-chart__list">
          {items.length === 0 ? (
            ...
            items.map((item) => {
              ...
              const width =
                resolvedMaxValue > 0 ? Math.max((numericValue / resolvedMaxValue) * 100, 6) : 0
