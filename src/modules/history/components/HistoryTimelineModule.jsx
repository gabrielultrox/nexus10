...
function buildCalendarDays(baseDayKey, eventsByDay) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    const tones = Array.from(
      new Set(dayEvents.map((event) => buildModuleTone(event.modulePath))),
    ).slice(0, 3)
...
function categorizeEvent(event) {
  ...
  if (action.includes('excluiu') || action.includes('removeu') || action.includes('cancelou')) {
    return 'delete'
  }

  if (action.includes('reset') || action.includes('limpou') || action.includes('reinici')) {
    return 'reset'
  }
...
function extractCourierFromLegacyText(event) {
  ...
  const patterns = [/(?:para|retornado para)\s+([^.,|]+)/i, /(?:entregador:)\s*([^.,|]+)/i]
...
function HistoryTimelineModule() {
  ...
  const groupedEvents = useMemo(() => groupEventsByHour(visibleEvents), [visibleEvents])
  ...
  const quickModuleChips = useMemo(() => {
    ...
    if (
      !PRIMARY_HISTORY_FILTERS.some((filter) => filter.id === activeFilter.id) &&
      activeFilter.id !== 'all'
    ) {
    ...
    <section className="history-module">
      ...
      <div className="history-module__filters">
        ...
        <div className="history-module__toolbar">
          ...
          <div
            className="history-module__quick-filters"
            role="tablist"
            aria-label="Filtro de tipo de evento"
          >
      ...
      <div className="history-module__layout">
                      ...
                      <span className="history-module__calendar-dots" aria-hidden="true">
                        {day.hasEntry
                          ? day.tones.map((tone) => (
                              <span
                                key={`${day.id}-${tone}`}
                                className={`history-module__calendar-dot history-module__calendar-dot--${tone}`}
                              />
                            ))
                          : null}
                      </span>
        ...
        <SurfaceCard
          title={selectedDay ? `Atividades de ${formatDayLabel(selectedDay)}` : 'Atividades'}
        >
          {visibleEvents.length === 0 ? (
            <EmptyState message="Nenhuma atividade neste recorte" />
          ) : (
            <div
              key={`${activeFilter.id}-${activeTypeFilter.id}-${selectedDay}-${searchQuery}`}
              className="history-module__timeline history-module__timeline--animated"
            >
              {groupedEvents.map((group) => {
                const isCollapsible = group.events.length > 5
                const isCollapsed = Boolean(collapsedHours[group.hour])
                const visibleGroupEvents =
                  isCollapsible && isCollapsed ? group.events.slice(0, 5) : group.events
                    ...
                    <div className="history-module__hour-events">
                      {visibleGroupEvents.map((event) => {
                        ...
                        return (
                          <article
                            key={event.id}
                            className={`history-module__entry history-module__entry--${category}`}
                          >
                            <span className="history-module__entry-time">
                            ...
                            <div className="history-module__entry-content">
                              <div className="history-module__entry-top">
                                <div className="history-module__entry-badges">
                                  <span
                                    className={`ui-badge ui-badge--${buildModuleTone(event.modulePath)}`}
                                  >
                                    {event.module}
                                  </span>
                                  {event.actor ? (
                                    <span className="history-module__actor">{event.actor}</span>
                                  ) : null}
                                </div>
                                {originLink ? (
                              ...
                              <p className="history-module__entry-description">
                              ...
                              <div className="history-module__entry-meta">
                                ...
                                <span>
                                  Tipo:{' '}
                                  {EVENT_TYPE_FILTERS.find((filter) => filter.id === category)
                                    ?.label ?? 'Atualizacoes'}
                                </span>
                              </div>
                              {event.details ? (
