...
export function invalidateQueryCache(matchers = []) {
  const normalizedMatchers = Array.isArray(matchers)
    ? matchers.filter(Boolean)
    : [matchers].filter(Boolean)
...
export async function getPaginatedStoreCollectionDocuments(
  ...
  const resolvedCacheKey = cacheKey ? `${cacheKey}::${cursor?.id ?? 'initial'}::${pageSize}` : null
  ...
  const result = {
    items,
    nextCursor:
      snapshot.docs.length < pageSize ? null : createPaginationCursor(lastVisible, orderField),
...
export async function getStoreDocumentsByIds(storeId, collectionName, documentIds = []) {
  ...
  const snapshots = await Promise.all(
    chunks.map((chunk) =>
      getDocs(createStoreScopedQuery(storeId, collectionName, [where(documentId(), 'in', chunk)])),
    ),
  )
