...
export function buildPdvReportData({
  ...
  const filteredFinancialEntries = financialEntries.filter((entry) =>
    isWithinPeriod(entry.createdAt, startDate, endDate),
  )
  ...
  const orderStatusSource =
    filteredOrders.length > 0
      ...
      : filteredSales.map((sale) => sale.orderSnapshot?.status).filter(Boolean)
  ...
  return {
    totals: {
      ...
      salesCount,
      estimatedProfit,
      activeFinancialSales: activeFinancialSales.reduce(
        (total, entry) => total + parseMoney(entry.amount),
        0,
      ),
    },
    topProducts,
