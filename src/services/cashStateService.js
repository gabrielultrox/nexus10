...
export function subscribeToCashState({ storeId, storageKey, resetHour = 3, onData, onError }) {
  ...
  return guardRemoteSubscription(
    () =>
      onSnapshot(
        ...
        (snapshot) => {
          const normalized = normalizeCashState(
...
export async function saveCashState({ storeId, tenantId, storageKey, state, resetHour = 3 }) {
