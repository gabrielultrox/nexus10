...
export function createIfoodAdapter({
  ...
  return {
    ...
    async getAccessToken({ clientId, clientSecret }) {
      return requestJson(
        `${resolvedConfig.authBaseUrl}/oauth/token`,
        {
          method: 'POST',
    ...
    async pollEvents({ accessToken }) {
      return requestJson(
        `${resolvedConfig.merchantBaseUrl}${resolvedConfig.pollingPath}`,
        {
          method: 'GET',
    ...
    async acknowledgeEvents({ accessToken, eventIds }) {
      return requestJson(
        `${resolvedConfig.merchantBaseUrl}${resolvedConfig.acknowledgmentPath}`,
        {
          method: 'POST',
          headers: buildJsonHeaders(accessToken),
