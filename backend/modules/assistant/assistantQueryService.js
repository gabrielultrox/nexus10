...
function extractSearchTerm(text, removableTokens = []) {
  return removableTokens.reduce(
    (currentText, token) => currentText.replace(token, ' '),
    text
      .replace(/\b(mostrar|buscar|procurar|listar|abrir|ir para|de hoje|do ifood|da ifood|dos|das)\b/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim(),
  )
    .replace(/\s+/g, ' ')
...
function isGenericSearchTerm(value) {
  return ['', 'pedido', 'pedidos', 'venda', 'vendas', 'cliente', 'clientes', 'produto', 'produtos'].includes(normalizeText(value));
}
...
export function createAssistantQueryService() {
  return {
    async searchOrders({ storeId, text }) {
      ...
      const filtered = buildFilteredRecords(records, text, buildOrderProjector, [/\bpedido[s]?\b/gi])
    ...
    async searchCustomers({ storeId, text }) {
      ...
      const filtered = buildFilteredRecords(records, text, buildCustomerProjector, [/\bcliente[s]?\b/gi]);
    ...
    async searchProducts({ storeId, text }) {
      ...
      const filtered = buildFilteredRecords(records, text, buildProductProjector, [/\bproduto[s]?\b/gi]);
